...
HRESULT InitDevice()
{
  ...
  hr = CompileShaderFromFile("PandoraCoreEngine.fx", "VS", "vs_4_0", &pVSBlob);
  if (FAILED(hr))
  {
    MessageBox(NULL,
  ...
  hr = CompileShaderFromFile("PandoraCoreEngine.fx", "PS", "ps_4_0", &pPSBlob);
  if (FAILED(hr))
  {
    MessageBox(NULL,
      "The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", "Error", MB_OK);
