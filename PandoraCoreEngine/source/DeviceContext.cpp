...
DeviceContext::VSSetShader(ID3D11VertexShader* pVertexShader,
  ...
  m_deviceContext->VSSetShader(pVertexShader, ppClassInstances,
                               NumClassInstances);
}
...
DeviceContext::PSSetShader(ID3D11PixelShader* pPixelShader,
  ...
  m_deviceContext->PSSetShader(pPixelShader, ppClassInstances,
                               NumClassInstances);
}
...
DeviceContext::IASetVertexBuffers(unsigned int StartSlot,
  ...
  if (!ppVertexBuffers || !pStrides || !pOffsets) {
    ERROR("DeviceContext", "IASetVertexBuffers",
          "Invalid arguments: ppVertexBuffers, pStrides, or pOffsets is "
          "nullptr");
    return;
  }
  m_deviceContext->IASetVertexBuffers(StartSlot, NumBuffers, ppVertexBuffers,
                                      pStrides, pOffsets);
}

void
DeviceContext::IASetIndexBuffer(ID3D11Buffer* pIndexBuffer, DXGI_FORMAT Format,
                                unsigned int Offset) {
  if (!pIndexBuffer) {
...
void
DeviceContext::PSSetSamplers(unsigned int StartSlot, unsigned int NumSamplers,
...
DeviceContext::OMSetRenderTargets(
    unsigned int NumViews, ID3D11RenderTargetView* const* ppRenderTargetViews,
...
DeviceContext::ClearDepthStencilView(ID3D11DepthStencilView* pDepthStencilView,
                                     unsigned int ClearFlags, float Depth,
                                     UINT8 Stencil) {
  ...
  if ((ClearFlags & (D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL)) == 0) {
    ERROR("DeviceContext", "ClearDepthStencilView",
          "Invalid ClearFlags: must include D3D11_CLEAR_DEPTH or "
          "D3D11_CLEAR_STENCIL");
    return;
  }
...
DeviceContext::VSSetConstantBuffers(unsigned int StartSlot,
  ...
  m_deviceContext->VSSetConstantBuffers(StartSlot, NumBuffers,
                                        ppConstantBuffers);
}
...
DeviceContext::PSSetConstantBuffers(unsigned int StartSlot,
  ...
  m_deviceContext->PSSetConstantBuffers(StartSlot, NumBuffers,
                                        ppConstantBuffers);
}
...
DeviceContext::DrawIndexed(unsigned int IndexCount,
  ...
  m_deviceContext->DrawIndexed(IndexCount, StartIndexLocation,
                               BaseVertexLocation);
}