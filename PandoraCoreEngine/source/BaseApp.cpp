...
int BaseApp::run(HINSTANCE hInst, int nCmdShow) {
	...
	// 2. Inicializar DirectX y Recursos
	if (FAILED(init())) {
		return 0;
	}
	...
	// IMPORTANTE: NO LLAMAR A destroy() AQUÍ.
	// El destructor de la clase se encargará de ello al salir del ámbito.
	// Si lo llamas aquí y luego en el destructor, crashea.
...
HRESULT BaseApp::init() {
	...
	if (!m_nissanActor.isNull()) {
		...
		if (SUCCEEDED(hr)) textures.push_back(nissanTexture);

		m_nissanActor->setMesh(m_device, modelMeshes);
		m_nissanActor->setTextures(textures);
		m_nissanActor->setName("Nissan GT-R");
		m_nissanActor->getComponent<Transform>()->setTransform(EU::Vector3(0,-1,0), EU::Vector3(0,0,0), EU::Vector3(1,1,1));
	...
	// Outline States
	if (FAILED(initOutlineStates())) return E_FAIL;
...
void BaseApp::render() {
	...
	m_deviceContext.m_deviceContext->ClearDepthStencilView(m_depthStencilView.m_depthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
	...
	for (int i = 0; i < m_actors.size(); ++i) {
		...
		if (i == m_selectedActorIndex) m_deviceContext.m_deviceContext->OMSetDepthStencilState(m_stencilStateWrite, 1);
		else m_deviceContext.m_deviceContext->OMSetDepthStencilState(nullptr, 0);
...
void BaseApp::renderOutline(int actorIndex) {
	...
	transform->setScale(EU::Vector3(originalScale.x * 1.02f, originalScale.y * 1.02f, originalScale.z * 1.02f));
...
void BaseApp::destroy() {
	if (!m_device.m_device) return; // Protección anti doble delete

	m_editor.destroy(); // ImGui shutdown primero
	...
	m_actors.clear(); // Limpiar smart pointers
...
LRESULT BaseApp::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if (pApp) {
		if (pApp->m_editor.WndProc(hWnd, message, wParam, lParam)) return true;
	}
	switch (message) {
	case WM_CREATE: {
