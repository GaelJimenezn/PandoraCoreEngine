...
Device::CreateTexture2D(const D3D11_TEXTURE2D_DESC* pDesc,
  ...
  if (SUCCEEDED(hr)) {
    ...
    ERROR("Device", "CreateTexture2D",
      ("Failed to create Texture2D. HRESULT: " 
       + std::to_string(hr)).c_str());
  }
  return hr;
}
...
Device::CreateVertexShader(const void* pShaderBytecode,
  ...
  if (BytecodeLength == 0) {
    ERROR("Device", "CreateVertexShader", "BytecodeLength is zero");
    return E_INVALIDARG;
  }
  if (!ppVertexShader) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    ERROR("Device", "CreateVertexShader",
      ("Failed to create Vertex Shader. HRESULT: " 
       + std::to_string(hr)).c_str());
  }
  return hr;
}
...
Device::CreateInputLayout(const D3D11_INPUT_ELEMENT_DESC* pInputElementDescs,
  ...
  if (NumElements == 0) {
    ERROR("Device", "CreateInputLayout", "NumElements is zero");
    return E_INVALIDARG;
  }
  ...
  if (BytecodeLength == 0) {
    ERROR("Device", "CreateInputLayout", "BytecodeLength is zero");
    return E_INVALIDARG;
  }
  if (!ppInputLayout) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    ERROR("Device", "CreateInputLayout",
      ("Failed to create Input Layout. HRESULT: " 
       + std::to_string(hr)).c_str());
  }
  return hr;
}
...
Device::CreatePixelShader(const void* pShaderBytecode,
  ...
  if (BytecodeLength == 0) {
    ERROR("Device", "CreatePixelShader", "BytecodeLength is zero");
    return E_INVALIDARG;
  }
  if (!ppPixelShader) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    ERROR("Device", "CreatePixelShader",
      ("Failed to create Pixel Shader. HRESULT: " 
       + std::to_string(hr)).c_str());
  }
  return hr;
}
...
Device::CreateBuffer(const D3D11_BUFFER_DESC* pDesc,
  ...
  if (SUCCEEDED(hr)) {
    ...
    ERROR("Device", "CreateBuffer",
      ("Failed to create Buffer. HRESULT: " 
       + std::to_string(hr)).c_str());
  }
  return hr;
}
...
Device::CreateSamplerState(const D3D11_SAMPLER_DESC* pSamplerDesc,
  ...
  if (SUCCEEDED(hr)) {
    ...
    ERROR("Device", "CreateSamplerState",
      ("Failed to create Sampler State. HRESULT: " 
       + std::to_string(hr)).c_str());
  }
  return hr;
}
